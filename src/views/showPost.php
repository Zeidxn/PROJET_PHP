...
$posts = $postControllerPost->getPostsPage($page, $postsPerPage);
$relationController = new RelationController($db);

?>
<?php foreach ($posts as $post):
        if ($post['isPublic'] == 0 && $relationController->isFriend($_SESSION['userId'], $post['owner']) !== "amis" && $post['owner'] === $_SESSION['userId']||$post['isPublic'] == 1||$post['isPublic']==0 && $relationController->isFriend($_SESSION['userId'], $post['owner']) === "amis"||$post['isPublic']==0 && $relationController->isFriend($post['owner'], $_SESSION['userId']) === "amis"):
?>
            <div class="post-container">
                ...
                if($comments!=null){
                    foreach ($comments as $comment) {
                        $user=$postControllerPost->getUserName($comment['userId']);
